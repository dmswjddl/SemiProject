					...
					<form action="updateBoard.do" method="post" enctype="multipart/form-data">
						...
						<div class="form-floating mb-3">
							<input type="text"` class="form-control" placeholder="Leave a comment here"
								id="floatingTextarea" style="height: 15px; align:left" name="boardTitle" value="${oneBoard.boardTitle}">
						</div>
						내 용
						<div class="form-floating">
							<textarea class="form-control" placeholder="Leave a comment here" id="floatingTextarea" name="boardContent" style="height: 300px">${oneBoard.boardTitle}</textarea>						
						</div>
						<!-- 파일 첨부 -->
						<div>
						<input class="form-control" type="text" placeholder="${oneBoard.boardFname}" aria-label="Disabled input example" disabled>
						<input class="form-control" type="file" id="file" name="file" value="${oneBoard.boardFname}">
						<button type="submit" class="btn btn-secondary m-2" id="btnDelete">파일삭제</button>	
						<a href="boardList.do"><button type="button" class="btn btn-secondary m-2" id="btnList">목록보기</button></a>
						<button type="submit" class="btn btn-secondary m-2" id="btnWrite">수정완료</button>					
						</div>											
					</form>
			</main>
